...
def main():
    t = 256
    step = t // 2
    path_folder = '/media/anderson/Jaiza/Roney/Rotulos/Pré-analisados'
    ...
    for file in orto_files:
        ...
        for x in tqdm(range(0, width-t, step)):
            ...
            for y in range(0, height-t, step):
                ...
                if prediction[x:x+t, y:y+t][0].shape[0] < 256:
                    print('Imagem menor')

                    patch_img = get_image_patch(dataset, x, y, prediction[x:x+t, y:y+t][0].shape[0], t)
                ...
                try: 
                    ...
                    if perc_pixels_brancos >= 0.95:
                     ...
                     # FAZER PRED
                    pred = predictionModel(patch_img)
                    ...
                    print('--------------------Erro ao calcular branco---------------------------')

            
        prediction = Image.fromarray(prediction)
        prediction.save(os.path.join('/home/anderson/Documentos/Roney/Código/ortosaida/fast/', f'{filename_orto}.tif'))
