...
def main():
    t = 256
    step = t // 2
    path_folder = '/home/anderson/Documentos/Roney/Rotulos_np/'
    ...
    for file in orto_files:
        ...
        try:
            path_daninha = os.listdir(os.path.join(path_folder,file,[i for i in path_orto if 'mamona' in i][1]))
            
        except:
            print("Não tem essa espécie")

        try:
            path_mask = os.path.join(path_folder,file,[i for i in path_orto if 'mamona' in i][1],[i for i in path_daninha if i.endswith('.shp')][0])
            print(path_mask)
        except:
        ...
        try:
            path_gt = os.listdir(os.path.join(path_folder,file,[i for i in path_orto if 'mask_mamona'in i][0]))
            path_gt = os.path.join(path_folder,file,[i for i in path_orto if 'mask_mamona' in i][0],[i for i in path_gt if i.endswith('.shp')][0])
            print(path_gt)
        except:
